...
// Correção para UUIDs de 128 bits
var (
...
type RealService struct {
	...
	// Controle de Loop FEC (Padrão Auuki)
	targetPower  float64
	targetGrade  float64
...
// initializeFEC segue a sequência exata do Auuki para habilitar o controle
func (s *RealService) initializeFEC() {
	...
	// 2. Wind Resistance (Página 50) - Auuki envia isso na inicialização
	time.Sleep(1000 * time.Millisecond)
	...
	// Inicia o loop de controle persistente
	go s.runControlLoop()
}
	...
	for {
		...
		case <-ticker.C:
			...
			if s.currentMode == "ERG" {
				...
				// No modo SIM, o Auuki envia a Página 51 repetidamente
...
func (s *RealService) SetGrade(grade float64) error {
	...
	// Se estiver em modo ERG, ignoramos o comando de Grade da rota
	if s.currentMode == "ERG" {
		return nil
	}
